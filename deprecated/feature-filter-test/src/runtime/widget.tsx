...
export default function Widget (props: AllWidgetProps<IMConfig> & ExtraProps) {
  ...
  function filterBtnHandler(evt) {
    // pointLayer.refresh()
    // console.log(pointLayer)
    const featureLayerView = view.layerViews.find(item => item.layer.title === "Deep Sea Coral and Sponge Observations")
    // console.log(featureLayerView)
    // featureLayerView.filter = new FeatureFilter({
    //   where: "DepthInMeters <= 50"
    // });
    pointLayer.definitionExpression = "((ScientificName LIKE '%pacifica%'))"
  }

  function refreshBtnHandler(evt) {
    // pointLayer.refresh()
    const defexp = pointLayer.definitionExpression
    console.log(defexp)
    const featureLayerView = view.layerViews.find(item => item.layer.title === "Deep Sea Coral and Sponge Observations")
    // console.log(featureLayerView)
    // featureLayerView.filter = new FeatureFilter({
    //   where: "DepthInMeters <= 50"
    // });
    pointLayer.definitionExpression = defexp
  }
  ...
  return (
    <div className="widget-use-map-view" style={{ width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{overflowY: 'auto', height: '100%', paddingLeft: '5px'}}>
        ...
        <button onClick={refreshBtnHandler}>Refresh Map</button>

        <div>
          {(isUpdating)? <span>map is updating...</span> : ''}
        </div>
      </div>
    </div>
  );
